  ...
  if (state === null || state !== storedState) {
    ...
    request.post(authOptions, function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        var options = {
          url: 'https://api.spotify.com/v1/users/1218489850/playlists/5EEyxxeU61V1qgR4VgiB0O',
        ...
        request.get(options, function(error, response, body) {
          ...
          body.tracks.items.forEach(function(el){
            ...
            x.split(" ").join("+");
            console.log(x);
            var requestURL = "http://developer.echonest.com/api/v4/artist/biographies?api_key=EVUVGQSETIVR4BVWC&name="+x+"&format=json&results=48&start=1&license=cc-by-sa"; 
            request.get(requestURL, function (err,response,body){
              console.log(i);
              var myData = JSON.parse(body);
              console.log(myData);
              var bio = myData.response.biographies[0].text;
              db.run("INSERT INTO tracks(artist_name, track_name, track_uri, album_name, album_url , artist_bio) VALUES (?,?,?,?,?,?)", el.track.artists[0].name, el.track.name, el.track.uri, el.track.album.name, el.track.album.images[0].url, bio ,function(err){
                if(err){
                  console.log(err);
                } else {
